...
class ApiClient(private val ctx: Context) {
     ...
     * --------- LOGIN -----------------------------------
     */
    fun login( username:String, password:String, completion: (token:String?, error: String?) -> Unit) {
        val route = ApiRoute.Login(username, password)
        this.performRequest(route) { success, response ->
            if (success) {
                ApiResponseManager(ctx).loginResponseManager(response.json, route.token, username, completion)
            } else {
